...
const RefUniverConst:React.FC = () => {
    const [data, setData] = useState<RefUniverResponse[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const result:RefUniverResponse[]  = await response.json();
                setData(result);
                setLoading(false);
            } catch (error) {
    ...
    return (
        <Container>
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', my: 4, width: '100%' }}>
